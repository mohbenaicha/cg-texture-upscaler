...
class TkListbox(ctk.CTkFrame):
    ...
    def setup_frame_elements(self, *args, **kwargs):
        ...
        if SearchConfig.last_used_dir_or_file:
            self.populate(SearchConfig.last_used_dir_or_file, SearchConfig.recursive)
    ...
    def handle_recursive_dir_walk(
        ...
        if update_lb:
            self.remove_items(clear_cache=False)
            self.configure_listbox_variable(listvariable=imcache[0])
    ...
    def populate(self, parent, recursive, add=False):
        def populate_task():
            imcache[0].clear()
            imcache[1].clear()
            ...
            if os.path.isdir(parent):
                if recursive:
                    self.handle_recursive_dir_walk(parent, imcache)
                else:
                    self.handle_single_dir(parent, imcache)
            else:  # otherwise, we're dealing with a file
                self.handle_single_file(parent, imcache)
    ...
    def filter(self, and_filters: List[str], or_filters: List[str], source_list: str):
        ...
        if not source_list == "current_list":
            ...
            if SearchConfig.last_used_dir_or_file != "":
                if SearchConfig.recursive:
                    self.handle_recursive_dir_walk(
                        SearchConfig.last_used_dir_or_file, imcache
                    )
                else:
                    self.handle_single_dir(SearchConfig.last_used_dir_or_file, imcache)
        ...
        while remaining > keep_idx:
            ...
            if (and_true and not len_and == 0) or (or_true and not or_true == 0):
                keep_idx += 1
            else:
                remaining -= 1
    ...
    def refresh(self, value):
        if SearchConfig.last_used_dir_or_file:
            ...
            self.populate(
                SearchConfig.last_used_dir_or_file, SearchConfig.recursive, add=False
            )
        else:
            print_to_frame(
                frame=self.button_frame,
